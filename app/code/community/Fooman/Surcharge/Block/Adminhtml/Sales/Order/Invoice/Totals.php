...
class Fooman_Surcharge_Block_Adminhtml_Sales_Order_Invoice_Totals extends Mage_Adminhtml_Block_Sales_Order_Invoice_Totals {
    ...
    public function initTotals() {
        if ($this->getSource()->getFoomanSurchargeAmount()!= 0){
            ...
            if ($parent->getTotal('subtotal_incl')) {
                $parent->getTotal('subtotal_incl')->setValue($this->getSource()->getSubtotal() + $this->getSource()->getTaxAmount() - $this->getSource()->getFoomanSurchargeTaxAmount());
                $parent->getTotal('subtotal_incl')->setBaseValue($this->getSource()->getBaseSubtotal() + $this->getSource()->getBaseTaxAmount() - $this->getSource()->getBaseFoomanSurchargeTaxAmount());

            } elseif (Mage::helper('tax')->displaySalesSubtotalInclTax()) {
                $parent->getTotal('subtotal')->setValue($parent->getTotal('subtotal')->getValue() - $this->getSource()->getFoomanSurchargeTaxAmount());
                $parent->getTotal('subtotal')->setBaseValue($parent->getTotal('subtotal')->getBaseValue() - $this->getSource()->getBaseFoomanSurchargeTaxAmount());
            }*/
        }
