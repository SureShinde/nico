...
class Webshopapps_Shipusa_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getUspsBoxWeighting($boxTypeId)
    {
     ...
     public function getPackageBreakdown()
    {
        $packages = Mage::getSingleton('shipusa/dimcalculate')->getBoxes($this->getQuote()->getAllItems());
    ...
    public function getPackageBreakdownText($packages) {
        ...
        foreach ($packages as $key=>$box)
        {
            ...
            if ($box!=null) {
                if(array_key_exists('package_name', $box)) {
    ...
    public function getProductBreakdownText($box) {
        $productText = '';
        if (array_key_exists('products',$box)  || (is_object($box) && !is_null($box->getProducts()))) {
            if (is_array($box['products'])) {
                foreach ($box['products'] as $product) {
                    $productText .= ' SKU='.$product.';  ';
                }
            } else {
                $productText = $box['products'];
            }
        }
        return $productText;
    }        
    ...
    public function isDimShippingAvailable($carrierCode, $countryCode)
    {
        if(Mage::getStoreConfig('carriers/'.$carrierCode.'/dim_allowspecific')) {
            $allowedCountries = explode(',' ,Mage::getStoreConfig('carriers/'.$carrierCode.'/dim_specificcountry'));
    ...
    public function getNoDimBox($request){
        return array(
            array(
                'height' => 0,
                'width' => 0,
