...
class Webshopapps_Shipusa_Model_Dimcalculate
{
    	...
    	foreach($items as $item) {
			...
			if ($item->isShipSeparately()) {
    			if (!Mage::helper('wsacommon/shipping')->getItemTotals($item, $weight,$qty,$price,$this->_useBundleParent, $ignoreFreeItems)) {
	       		...
	       		$params=$this->getProductParams($item,$this->_useBundleParent,$qty,$weight,$price);
			} else {
				if (!Mage::helper('wsacommon/shipping')->getItemTotals($item, $weight,$qty,$price,$this->_useBundleParent, $ignoreFreeItems)) {
					continue;
				}
            ...
            if($processingFlat){
                ...
                switch ($packingAlgorithm) {
                    ...
                    case 'best_fit_packing':
                        if (!empty($params['possible_ship_boxes'])) {
                            $bestFitArr[] = $params;
                        } elseif (!empty($params['multiple_boxes'])) {
	 ...
	 *
	 **/
	protected function getProductParams($item,$useParent,$qty,$weight,$price)
    {
        $product = null;
        if (Mage::helper('shipusa')->isDebug()) {
