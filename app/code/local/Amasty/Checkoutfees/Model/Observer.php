...
class Amasty_Checkoutfees_Model_Observer {
    /* HCL-109: fix issue for showing checkout fee on account order view */

    /**
     * get full action name for the current request
     */
    ...
    public function insertHtml($observer) {
        ...
        if (((strpos($class, '_Email_') !== false) || in_array($currentAction, $validActions)) && !Mage::registry('amcheckoutfees_email_template_style')) {
    ...
    public function getFeesLabel($source, $quote = null) {
        $label = array();
        if ((float) $source->getBaseAmcheckoutfeesAmount()) {
            $orderId = $source->getIncrementId();
            $fees = $source->getAmcheckoutfeesFees();
            $orderId = $source->getIncrementId();
            if (empty($orderId)) {
                $quote = Mage::getSingleton('checkout/session')->getQuote();
                $fees = $quote->getAmcheckoutfeesFees();
            }
            $currency = $source->getBaseCurrencyCode();
            try {
    ...
    private function _prepareTotalsHtml($block, $html) {
        ...
        $labels = $this->getFeesLabel($order);
        if (!empty($labels) && is_array($labels)) {
            $labelStr = implode(",", $labels);
        }

        //check for the availability for action object as in case of cron jobs it's null
        if (is_object($block->getAction())) {
        ...
        if (!empty($amount) && $amount != "0.00") {
            ...
            switch ($action) {
                ...
                case 'sales_order_view':
                    ...
                    $html_block = '
						<tr class="">
							<td colspan="4" class="a-right">' . Mage::helper('amcheckoutfees')->__('Additional Fees(' . $labelStr . ')') . '</td>
                ...
                case 'sales_order_creditmemo':
                    ...
                    $html_block = '
						<tr class="">
							<td colspan="6" class="a-right">' . Mage::helper('amcheckoutfees')->__('Additional Fees(' . $labelStr . ')') . '</td>
                ...
                case 'sales_order_printCreditmemo':
                    ...
                    $html_block = '
						<tr class="">
							<td colspan="6" align="right" style="font-size:13px;padding:3px 9px">' . Mage::helper('amcheckoutfees')->__('Additional Fees(' . $labelStr . ')') . '</td>
                ...
                case 'sales_order_printInvoice':
                    ...
                    $html_block = '
						<tr class="">
							<td colspan="4" align="right" style="font-size:13px;padding:3px 9px">' . Mage::helper('amcheckoutfees')->__('Additional Fees(' . $labelStr . ')') . '</td>
							<td colspan="2" align="right"  style="font-size:13px; padding:3px 9px;"><span class="price">' . $amount . '</span></td>
						</tr>
					';
    ...
    private function getDataByParams() {
         ...
         * case: when cron runs
         */ elseif (empty($params)) {
            $orderId = Mage::getSingleton('core/session')->getOrderIdForCron();
            if (isset($orderId) && !empty($orderId)) {
                $data = Mage::getModel('sales/order')->load($orderId);
                // Mage::getSingleton('core/session')->unsOrderIdForCron();
            } else {
                $data = array();
            }
        } /*
    ...
    public function beforePaymentMethodSave($observer) {
        try {
            ...
            Mage::log("Error In checkout fee Observer. " . $e->getMessage());
        }
    }
