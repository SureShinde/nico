            ...
            foreach ($storedData as $storedFeesId => $storedFeesData) {
                ...
                if ($storedFeeData->getSize()) {
                    foreach ($storedFeeData as $fee) {
                        if($fee->getPriceType() != 2){
                        if ($fee->getPriceType() == 1) {
                         ...
                         $amountCustom += Mage::helper('amcheckoutfees')->getAdditionalWireTransferFee($fee->getPrice(),$fee->getPriceType());
                         $baseCustom += Mage::helper('amcheckoutfees')->getAdditionalWireTransferFee($fee->getPrice(),$fee->getPriceType());
                        }
                    }
                }
            }
        }
        $amountFinal = $amount + $amountCustom;
        $baseFinal = $amount + $baseCustom;
        if ($amountFinal) {
            $this->_setBaseAmount($amountFinal);
            $this->_setAmount($baseFinal);
        }
        ...
        */
        ...
        if ($amcheckoutfeesAmount) {
            $address->addTotal(
                array(
                    "code"  => $this->getCode(),
                    "title" => Mage::helper('amcheckoutfees')->__("Additional Fees"),
