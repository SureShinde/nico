...
class Amasty_Customerattr_Block_Customer_Fields extends Mage_Core_Block_Template {
    ...
    public function needRelations() {
        if ($this->_hasRequired || $this->_hasValidation || 0 < Mage::helper('amcustomerattr')->getElementsRelation()
                        ->getSize()
        ) {
            return true;
        }
    ...
    public function getFormElements() {
        ...
        foreach ($attributes as $attribute) {
            if (!empty($attributesToRender) && !in_array(
            ...
            $fieldName = $this->_isCheckout() ? 'billing[amcustomerattr][' . $attribute->getAttributeCode()
            ...
            if ('selectgroup' == $typeInternal) {
                if (Mage::getSingleton('customer/session')->isLoggedIn() && !Mage::getStoreConfig(
            ...
            /* set padding 0px '<div style="padding: 4px;"></div>'; */
            if ('file' == $inputType) {
            ...
            if ($inputType == 'select' || $inputType == 'selectimg' || $inputType == 'multiselect' || $inputType == 'multiselectimg'
                ...
                $dateImage = $this->getSkinUrl(
                        'images/grid-cal.gif', array('_area' => 'adminhtml', '_package' => 'default')
                );
    ...
    protected function _isRegistration() {
        ...
        if ($store_id == '5') {
            ...
            /* HCL-52 : get attribute field on login page */
            return ('login' == Mage::app()->getRequest()->getActionName());
        }
    }
