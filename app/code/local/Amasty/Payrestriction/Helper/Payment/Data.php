...
class Amasty_Payrestriction_Helper_Payment_Data extends Mage_Payment_Helper_Data {
    ...
    public function getStoreMethods($store = null, $quote = null) {
        ...
        foreach ($methods as $k => $method) {
            ...
            foreach ($this->getRules($address, $items) as $rule) {
                if ($rule->restrict($method)) {
                    if ($rule->validate($address) && $this->isCouponValid($quote, $rule) && !$this->isCouponValid($quote, $rule, true)
                    ) {
                        unset($methods[$k]);
                    }//if validate
                }//if restrict
        ...
        if (Mage::getSingleton('customer/session')->isLoggedIn()) {
            ...
            $selectedallowedIds = $customerInfo->getData('allowed_payment_methods');
			
            $attributeId = Mage::getResourceModel('eav/entity_attribute')->getIdByCode('customer', 'allowed_payment_methods');
            $collection = Mage::getResourceModel('eav/entity_attribute_option_collection')
                    ->setPositionOrder('asc')
                    ->setAttributeFilter($attributeId)
                    ->setStoreFilter(0)
                    ->load();
            $optArr = $collection->toOptionArray();
			
            $assOptArr = array();
            if (isset($optArr) && !empty($optArr)) {
                foreach ($optArr as $index => $optSubArr) {
                    $assOptArr[$optSubArr['value']] = $optSubArr['label'];
                }
            }
			
			
            if (isset($selectedallowedIds) && !empty($selectedallowedIds)) {
                $explodedArr = explode(',', $selectedallowedIds);
            }
			
            $selectedOptions = array();
            if (!empty($explodedArr) && $assOptArr) {

                foreach ($explodedArr as $index => $id) {
                    $selectedOptions[$id] = $assOptArr[$id];
                }
            }
        }
		
		if(isset($selectedOptions) && !empty($selectedOptions)){
			$allSelectedCodes = array();
			foreach($selectedOptions as $optionId => $optionlabel){
				$explodedLabelArr = explode('(',$optionlabel);
				if(isset($explodedLabelArr) && ( count($explodedLabelArr)>1 ) ){
					 $methodCode = str_replace(')', "", $explodedLabelArr['1']);
					 $allSelectedCodes[] = trim($methodCode," ");
				}
			}
		}
		
		if((!empty($store)) && ( 1 == $store )){   
			foreach ($methods as $index => $methodObj) {
				$code = $methodObj->getCode();
				 if (isset($allSelectedCodes) && !empty($allSelectedCodes)) {
					if (!in_array($code, $allSelectedCodes)) {
						unset($methods[$index]);
					 }
				}
			}  
