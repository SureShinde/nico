...
class Amasty_Shiprestriction_Model_Observer {
    ...
    public function restrictRates($observer) {
        ...
        if (Mage::getSingleton('customer/session')->isLoggedIn()) {
            ...
            $selectedallowedIds = $customerInfo->getData('allowed_shipment_methods');
            $attributeId = Mage::getResourceModel('eav/entity_attribute')->getIdByCode('customer', 'allowed_shipment_methods');
        ...
        foreach ($rates as $rate) {
            if (is_object($rate)) {
                ...
                if ($carrier == 'fedex') {
                    ...
                    $codeLevel = 'USPS';
                } elseif ($carrier == 'umosaco') {
                    $codeLevel = 'Use My Account';
                } elseif ($carrier == 'dhl') {
                    $codeLevel = 'DHL';
                } elseif ($carrier == 'freeshipping') {
                    $codeLevel = 'Free Shipping';
                }
            }
            if (isset($selectedOptions) && !empty($selectedOptions) && !empty($codeLevel)) {
                if (!in_array($codeLevel, $selectedOptions)) {
                    continue;
                }
            }
        ...
        if ($isEmptyResult || ($isShowMessage && $isRestrict)) {
            if (isset($lastRate) && !empty($lastRate)) {
    ...
    protected function _getRestrictionRules($request) {
        ...
        if (!$address) {
            ...
            if (!$quote) {
                return array();
            } // we need it for true order editor
        ...
        foreach ($this->_allRules as $rule) {
            ...
            if ($validBackOrder && $rule->validate($address) && $this->isCouponValid($request, $rule) && !$this->isCouponValid($request, $rule, true)
            ) {
