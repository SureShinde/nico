...
class Emjainteractive_ShippingOption_Block_Renderer_Text extends Emjainteractive_ShippingOption_Block_Renderer_Abstract
{
    ...
    protected function _renderElement()
    {
        $req = $this->_getIsRequired() ? true : false;
        if ($req) {
            $html = '<input class="input-text required-entry" type="text" name="' . $this->_getElementName() . '" id="' . $this->_getElementId() . '" value="' . $this->getOption()->getDefaultValue() . '" />';
        }
        else {
            $html = '<input class="input-text" type="text" name="' . $this->_getElementName() . '" id="' . $this->_getElementId() . '" value="' . $this->getOption()->getDefaultValue() . '" />';
        }
        return $html;
    }
}