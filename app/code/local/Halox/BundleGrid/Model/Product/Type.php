...
/**
 * Overridden Bundle product type class
 */

class Halox_BundleGrid_Model_Product_Type extends Mage_Bundle_Model_Product_Type
{

	/**
     * Replica of the prepareProduct method from Mage_Catalog_Model_Product_Type_Abstract
     *
     ...
     * @return array|string
     */
    protected function _grandParentPrepareProduct(Varien_Object $buyRequest, $product, $processMode)
    {
	...
	/**
	 * override this method to add to cart bundle product with custom post data due to grid options
	 */
	protected function _prepareProduct(Varien_Object $buyRequest, $product, $processMode)
	{
		$result = $this->_grandParentPrepareProduct($buyRequest, $product, $processMode);
        ...
        if (is_array($options)) {
            ...
            foreach ($options as $_optionId => $_selections) {
                ...
                //remove option id from bundle_options if no selecitons have been made for them
                }elseif(is_array($_selections)){
                    $_selections = array_filter($_selections, 'intval');
                    if(empty($_selections)){
                        unset($options[$_optionId]);
                    }
                }
        ...
        if (count($selections) > 0 || !$isStrictProcessMode) {
            ...
            foreach ($selections as $selection) {
                if ($selection->getSelectionCanChangeQty() && isset($qtys[$selection->getOptionId()])) {
                   	...
                   	/**
                     * for grid option type read qtys per selection for each option
                     */
                    if($selection->getOption()->getType() == 'grid'){
                   		$qty = (float)$qtys[$selection->getOptionId()][$selection->getSelectionId()] > 0 ? $qtys[$selection->getOptionId()][$selection->getSelectionId()] : 0;
                   	}else{
                   		$qty = (float)$qtys[$selection->getOptionId()] > 0 ? $qtys[$selection->getOptionId()] : 0;	
                   	}

				} else {
                    $qty = (float)$selection->getSelectionQty() ? $selection->getSelectionQty() : 0;
                }
