...
class Mage_Sitemap_Model_Sitemap extends Mage_Core_Model_Abstract
{
    ...
    public function generateXml()
    {
        ...
        foreach ($categories->getItems() as $item) {
            if ($baseUrl . $item->getUrl() == 'bestseller') continue;
            if (strpos($baseUrl . $item->getUrl(),'blog') !== false)  continue;
            if (preg_match('/blog/',$baseUrl . $item->getUrl()))     continue;
            if (preg_match('/bestseller/',$baseUrl . $item->getUrl()))     continue;
            if (preg_match('/g6-tan-cartomizers/',$baseUrl . $item->getUrl()))     continue;
            if (preg_match('/search-results/',$baseUrl . $item->getUrl()))     continue;
            $xml = sprintf(
    ...
    foreach ($collection as $item) {
       ...
       // Mage::log($item->getUrl(), NULL, 'SeoSitemap.log');
        ...
        if ($item->getUrl() == 'thank-you.html') continue;
        if ($item->getUrl() == 'enable-cookies') continue;
        if ($item->getUrl() == 'web-to-lead.html') continue;
        if ($item->getUrl() == 'bestseller.html') continue;
        if ($item->getUrl() == 'thank-you') continue;        
        if ($item->getUrl() == 'web-to-lead') continue;
        
        if ($item->getUrl() == 'no-route-2') continue;
        if ($item->getUrl() == 'no-route') continue;
        if (strpos($item->getUrl(),'blog') !== false)  continue;
        if (preg_match('/blog/',$item->getUrl()))     continue;
        if (preg_match('/bestseller/',$baseUrl . $item->getUrl()))     continue;
        if (preg_match('/g6-tan-cartomizers/',$baseUrl . $item->getUrl()))     continue;
        if (preg_match('/search-results/',$baseUrl . $item->getUrl()))     continue;
     //   print_r($item->getUrl());exit;
