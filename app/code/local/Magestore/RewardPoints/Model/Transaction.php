...
class Magestore_RewardPoints_Model_Transaction extends Mage_Core_Model_Abstract {
    ...
    public function sendUpdateBalanceEmail($rewardAccount = null) {
		...
		if(isset($customerStoreId) && !empty($customerStoreId)){
			$store = Mage::app()->getStore($customerStoreId);
		}else{
        ...
        Mage::getModel('core/email_template')
                ...
                Mage::getStoreConfig(self::XML_PATH_EMAIL_UPDATE_BALANCE_TPL, $store->getId()), Mage::getStoreConfig(self::XML_PATH_EMAIL_SENDER, $store), $customer->getEmail(), $customer->getName(), array(
            'store' => $store,
            'customer' => $customer,
            'title' => $this->getTitle(),
    ...
    public function sendBeforeExpireEmail() {
        ...
        Mage::getModel('core/email_template')
                ...
                Mage::getStoreConfig(self::XML_PATH_EMAIL_BEFORE_EXPIRE_TPL, $store->getId()), Mage::getStoreConfig(self::XML_PATH_EMAIL_SENDER, $store), $customer->getEmail(), $customer->getName(), array(
            'store' => $store,
            'customer' => $customer,
            'title' => $this->getTitle(),
            'amount' => $this->getPointAmount(),
