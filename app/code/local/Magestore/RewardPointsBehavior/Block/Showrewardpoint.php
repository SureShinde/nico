...
class Magestore_RewardPointsBehavior_Block_Showrewardpoint extends Mage_Core_Block_Template {
    ...
    public function getRewardViewProductInFo() {
        ...
        if ($this->isRateReview() == 2) {
            ...
            if ($this->_helper()->getRateConfig('show_rate_product') && $this->_helper()->getRateConfig('rate')) {
                ...
                $message = $this->_helper()->__('You will receive %s upon approval of this review and/or rating.', $pointreview);
            }
        }
        return $message;
    }
    ...
    public function getSocialInfo($chanel = null) {
        ...
        if(!$chanel){
            $offer_html = '<a href="'.Mage::getUrl('customer/account/login').'">Login or create an account</a> to earn Halo Points for sharing!';
            if(count($buttonShows) > 1) return $offer_html;
    ...
    public function getMessageSocial($buttonShows, $chanel = null) {  
        if (array_key_exists($chanel, $buttonShows)) {         
            switch ($chanel) {
                case 'facebook':
                    return $this->_helper()->__('Earn %s for liking this on Facebook!.', $buttonShows[$chanel]);
                case 'facebook_share':
                    return $this->_helper()->__('Earn %s for sharing this on Facebook!', $buttonShows[$chanel]);
                case 'twitter': 
                    return $this->_helper()->__('Earn %s for tweeting this on Twitter!.', $buttonShows[$chanel]);
                case 'googleplus':
                    return $this->_helper()->__('Earn %s for +1ing this on Google+!', $buttonShows[$chanel]);
                case 'pinterest':
