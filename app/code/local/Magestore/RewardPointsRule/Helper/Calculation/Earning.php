...
//        }
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                ...
                if ($rule->getActions()->validate($item)) {
                    ...
                    if ($rule->getSimpleAction() == 'by_qty' || $rule->getSimpleAction() == 'fixed') {
					    ...
					    $realItemEarning = 0;
						$rtotal = $rowTotal + $deltaRound;
					    if(!empty($rtotal)){
                        $realItemEarning = $baseItemPrice * $points / $rtotal;
                        }
					}
                    $itemEarning = Mage::helper('rewardpoints/calculator')->round($realItemEarning);
                    $deltaRound = $realItemEarning - $itemEarning;
    ...
    public function calcSalesPoints($pointOperation, $xAmount, $yStep, $price, $qtyStep, $qty, $maxPoint) {
        ...
        switch ($pointOperation) {
            ...
            case 'by_total':
                if ($yStep > 0) {
                    /* Remove round function and add floor in below function by chetu. 
                     * To resolve conflict subtoal earning point.
                     */
					$ydata = floor((string)($price / $yStep));
                    $points =  $ydata * $xAmount;					   
