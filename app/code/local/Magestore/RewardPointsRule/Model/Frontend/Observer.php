...
//            if (!$points || $points <= 0) return $this;
    ...
    public function sales_quote_item_save_before($observer){
        ...
        /*******************************************************************
        * REMOVED THIS CODE AS IT WAS THROWING THE NOT ENOUGH POINT EXCEPTION
        * EVEN THOUGH CUSTOMER HAD ENOUGH POINTS
        ********************************************************************/
        ...
        if($qty){
            ...
            //$prevItemPrice = $item->getProduct()->getFinalPrice(); 
            ...
            $catalogRules[$item->getId()] = array(
                ...
                'point_discount'        => null,//$prevItemPrice,
                'type'      => 'catalog_spend'
            );
            ...
            $session->setRewardError(Mage::helper('rewardpoints')->__('Reward products can only be redeemed with Points. You do not have enough Reward Points to redeem this item.'));
            Mage::throwException(Mage::helper('rewardpoints')->__('Reward products can only be redeemed with Points. You do not have enough Reward Points to redeem this item.'));
    ...
    public function controller_action_predispatch($observer){
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            /**
             * CONDITION HAS BEEN GIVEN A SEPERATE SECTION TO UNSET ITEM FROM CATALOG RULE ARRAY
             * SO THAT IF ITEM HAD POINTS REMOVED IT SHOULD NOT BE VISIBLE ON CART ITEM EITHER
             */
            if(!is_object($product) || !$product->getId() /*|| !$product->getRewardpointsSpend()*/){ 
            ...
            if(!$product->getRewardpointsSpend()){
                ...
                if(isset($catalogRules[$item->getId()]['type']) 
                    && $catalogRules[$item->getId()]['type'] == 'catalog_spend'
                ){
                    
                    //unset catalog rule for the items that has backend amount removed
                    unset($catalogRules[$item->getId()]);
                    $session->setCatalogRules($catalogRules);    

                    //restore the product price
                    $item->setPrice($product->getFinalPrice());
                    $item->setOriginalCustomPrice($product->getFinalPrice());
                    $item->getProduct()->setIsSuperMode(true);
