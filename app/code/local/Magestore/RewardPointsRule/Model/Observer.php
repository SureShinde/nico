...
class Magestore_RewardPointsRule_Model_Observer
{
    ...
    public function calculateSpendingRuleMaxPoints($observer)
    {
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    ...
                    }elseif($rule->getDiscountStyle() == 'by_item_price'){
                        $rowTotals[] = array(
                            'row_discount_amount' => $helper->_getItemBasePrice($child),
                            'row_total' => $item->getQty() * ($child->getQty() * $helper->_getItemBasePrice($child))
                        );
                    }
                }
                ...
                if (!$rule->getActions()->validate($item)) {
                    ...
                    $rowTotals[] = array(
                        'row_discount_amount' => $helper->_getItemBasePrice($item),
                        'row_total' => $item->getQty() * $helper->_getItemBasePrice($item)
                    );
                }
            }
        }
        ...
        if (in_array($rule->getDiscountStyle(), array('cart_fixed', 'by_item_price'))) {
            if ((float)$rule->getDiscountAmount()) {                
                ...
                if($baseTotal > $maxPriceSpend && $maxPriceSpend > 0) $baseTotal = $maxPriceSpend;
                 /* HCL-136 reward point spending issue */
                //$maxPoints = ceil($baseTotal / $rule->getDiscountAmount()) * $rule->getPointsSpended();
                $maxPoints = floor((string)($baseTotal / $rule->getDiscountAmount())) * $rule->getPointsSpended();
            }elseif($rule->getDiscountStyle() == 'by_item_price'){
                if($rule->getMaxPriceSpendedType() == 'by_price'){
                    $maxPriceSpend = $maxPrice;
                }elseif($rule->getMaxPriceSpendedType() == 'by_percent'){
                ...
                if($baseTotal > $maxPriceSpend && $maxPriceSpend > 0){ 
                    $baseTotal = $maxPriceSpend;
                }
                
                $maxPoints = 0.00;
                foreach($rowTotals as $total){
                    $maxPoints += ceil((string)$total['row_total'] / $total['row_discount_amount']) * $rule->getPointsSpended();
                }
            }
        } else {
            if ((float)$rule->getDiscountAmount()) {
                ...
                $maxPoints = floor((string)($percent / $rule->getDiscountAmount())) * $rule->getPointsSpended();
            }
        }
    ...
    protected function _sortQuoteItemsByPrice($a, $b)
    {
        $helper = Mage::helper('rewardpointsrule/calculation_spending');
        ...
        if($a->getItemPrice() < $b->getItemPrice()){
            return 1;
        }elseif($a->getItemPrice() > $b->getItemPrice()){
            return -1;
        }else{
            return 0;
        }
    ...
    /**
     * only works when discount style is 'by_item_price'
     * calculate possible discount based on the current quote items and their prices
     * give priority to items having higher price
     */
    protected function _getPossibleDiscountByQuote($quote, $rule, $points)
    {
        $discountPossible = 0;
        
        if($rule->getDiscountStyle() != 'by_item_price'){
            return $discountPossible;
        }
        
        $helper = Mage::helper('rewardpointsrule/calculation_spending');
        
        $validQuoteItems = array();
        
        //skip child quote items
        foreach($quote->getAllItems() as $item){
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    if ($rule->getActions()->validate($child)) {
                        ...
                        $item->setItemPrice($helper->_getItemBasePrice($child));
                    
                        $validQuoteItems[] = $item;
                    }
                }
            } elseif ($item->getProduct()) {
                if ($rule->getActions()->validate($item)) {
                    
                    $item->setItemPrice($helper->_getItemBasePrice($item));
        ...
        //sort quote items by price so that items having higher price can get the discount first
        usort($validQuoteItems, array($this, '_sortQuoteItemsByPrice'));
        
        $pointsClaimed = 0;
        foreach ($validQuoteItems as $_item) {
            if($pointsClaimed >= $points){
                break;
            }

            for($i=1;$i<=$_item->getQty();$i++){
                if($pointsClaimed >= $points){
                    break;
                }
                $discountPossible += $_item->getItemPrice();
                $pointsClaimed += $rule->getPointsSpended();
            }
        }
    ...
    public function calculateSpendingRuleDiscount($observer)
    {
        ...
        if ($availableTotal <= 0) {
            $points     = 0;
            $discount   = 0;
        } else if (in_array($rule->getDiscountStyle(), array('cart_fixed', 'by_item_price'))) {
            if ($rule->getPointsSpended() <= 0) {
                ...
                if ($rule->getDiscountAmount() > 0) {
                    ...
                    $possiblePoints = 0;
                    foreach($rowTotals as $total){
                        $possiblePoints += ceil((string)$total['row_total'] / $total['row_discount_amount']) * $rule->getPointsSpended();
                    }
                    
                    $points = min($points, $possiblePoints);
                    $points = floor($points / $rule->getPointsSpended()) * $rule->getPointsSpended();
                    
                    $discountPossible = $this->_getPossibleDiscountByQuote($quote, $rule, $points);
                    
                    $discount = min($discountPossible, $availableTotal);
