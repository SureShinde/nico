...
class Magestore_RewardPointsRule_Adminhtml_Reward_Spending_IndividualController extends Mage_Adminhtml_Controller_Action {
    ...
    public function ajaxSaveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $website = $this->getRequest()->getParam('website');
            $websiteScope = Mage::getModel('core/website')->getCollection()->toOptionHash();

            $id = isset($data['entity']) ? (int) $data['entity'] : null;
            $point = isset($data['value']) ? $data['value'] : null;
            if (strtolower($point) == 'empty')
                $point = '';
            $out = array(
                'message' => '',
                'value' => $point
            ...
            if ($point != '' && (!is_numeric($point) || $point < 0)) {
                ...
                try {
                    $storeId = '';
                    if (strtolower(trim($website)) == 'retail') {
                        $storeId = Mage::helper('rewardpointsrule')->getStoreIdByCode('halo_retail_english');

                        $product = Mage::getModel('catalog/product')->setStoreId($storeId)->load($id);
                    } else if (strtolower(trim($website)) == 'wholesale') {
                        $storeId = Mage::helper('rewardpointsrule')->getStoreIdByCode('halo_wholesale_english');
                        $product = Mage::getModel('catalog/product')->setStoreId($storeId)->load($id);
                    } else {
                        $storeId = 0;
                    ...
                    if ($product && $product->getId()) {
                        $product->setRewardpointsSpend($point)
                                ->save();
                        $out['value'] = Mage::getModel('catalog/product')->setStoreId($storeId)->load($id)->getRewardpointsSpend();
                    } else {
