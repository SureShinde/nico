<?php
class Smartwave_Megamenu_Block_Navigation extends MageParts_RestrictCategories_Block_Catalog_Navigation
{
            ...
            if (is_object($child) && $child->getIsActive())
            {
                ...
                if ($level == 1) {
					...
					if ($type == 'wide' && $catModel->getThumbnail()) {
						...
						$html .= '<img class="cloudzoom lazy" data-src="' . Mage::getBaseUrl('media').'catalog/category/' . $catModel->getThumbnail() . '" alt="' . $name.'"  />';
						$html .= '</a>';
						$html .= '</div>';
					}
					//version 1.0.2 end
...
//        if ((int)Mage::getStoreConfig('megamenu/columns/integrate') && count($target))
    ...
    /**
     * Fix to make this extension compatible with varnish
     */
    protected function _toHtml() {
        if( Mage::getStoreConfig('megamenu/general/enable') &&
                $this->getNameInLayout() == 'catalog.topnav' ) {
            $this->setTemplate('smartwave/megamenu/html/topmenu.phtml');
        }
