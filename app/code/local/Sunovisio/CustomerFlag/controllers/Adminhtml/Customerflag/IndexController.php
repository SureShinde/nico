...
class Sunovisio_CustomerFlag_Adminhtml_Customerflag_IndexController extends Mage_Adminhtml_Controller_Action {
 ...
 protected function _isAllowed()
    ...
    public function getnextAction () {
        $cId = $this->getRequest()->getParam('customerid');
        if(isset($cId) && !empty($cId)){
            //code for registered user[HCL-13]
        $customer = Mage::getModel('customer/customer')->load($this->getRequest()->getParam('customerid'));
        }else{
            //code for guest user[HCL-13]
        ...
        $customer->save();
            ...
            //code for guest user[HCL-13]
            $order->setData('customer_flag',$newFlag->getId());
            $order->save();
        }
        ...
        $content .= '<a href="javascript: changeFlag(' . $customer->getCustomerFlag() . ')"><img src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $newFlag->getPicture() . '" title="' . $newFlag->getLabel() . '" /></a>';
            ...
            //code for guest user[HCL-13]
          $content .= '<a href="javascript: changeFlag(' . $order->getData('customer_flag') . ')"><img src="' . Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $newFlag->getPicture() . '" title="' . $newFlag->getLabel() . '" /></a>';  
        }
        
        
        $content .= '</span>';
        
        echo json_encode(array('content' => $content));exit;
    }
}
