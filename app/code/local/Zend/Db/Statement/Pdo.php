...
class Zend_Db_Statement_Pdo extends Zend_Db_Statement implements IteratorAggregate
{
     ...
     * @throws Zend_Db_Statement_Exception
     */
     public function createErrorLog($error = null){
        try {
        if(!empty($error)){
         $path = "./var/log/";
		 if (!file_exists($path)) {
		 ...
		 $log = fopen($path."deadlock_error.log", "a");
		 fwrite($log, "\n". $error);
		 fclose($log);
        }
        }catch (Exception $e) {
               // error in writing log
        }
        
    }
    public function _execute(array $params = null) {
        $isEnabled = Mage::registry('deadlock_retry/enable/retry');        
        $max_tries = 2;
        if ($isEnabled == 1) {
            $max_tries = Mage::registry('deadlock_retry/enable/max_retry');
            $isLogEnabled = Mage::registry('deadlock_retry/enable/log');
            //$this->createErrorLog("Enabled Log".$isEnabled. 'Max rety'. $max_tries. "-log-". $isLogEnabled);           
             if(empty($max_tries)) {
                $max_tries = 2;
            }
            $tries = 0;
            do {
                ...
                try {
                    ...
                    if ($tries < $max_tries && preg_match('/SQLSTATE\[40001\]/', $e->getMessage())) {
                        if ($isLogEnabled == 1) {
                           $this->createErrorLog('deadlock at ' . date('Y-m-d h:i:s') . 'Error' . $e->getMessage());           
                       }
                        $retry = true;
                        sleep(1);
                    } else {
                        if ($isLogEnabled == 1) {
                            $this->createErrorLog('deadlock After Max Retry Zend_Db_Statement_Exception at ' . date('Y-m-d h:i:s') . 'Error' . $e->getMessage());
