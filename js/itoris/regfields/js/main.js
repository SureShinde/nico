...
ItorisHelper.reloadCaptcha = function(id, captchaType){
	$(id).src = ItorisHelper.baseUrl + 'captcha/'+captchaType+'?' + Math.random();
};

ItorisHelper.validateForm = function(formId) {
...
ItorisHelper.validateElements = function(elms, valid) {
	var elmValid = true;
...
ItorisHelper.showFileInput = function(inputId, confirmMessage, isRequired) {
...
ItorisHelper.updateZip = function() {
	ItorisHelper._updateZip('');
}

ItorisHelper._updateZip = function(prefix) {
	if ($(prefix + 'postcode')) {
		if (countriesWithOptionalZip.indexOf($(prefix + 'country_id').value) != -1) {
