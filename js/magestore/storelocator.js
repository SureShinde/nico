...
function radiusLocator(sort) {
    ...
    if (sort_store == 'distance' && sort) {
        ...
        var listStore =[];
        $$("#list-store li").each(function(el){
               listStore.push(el.clone(true));
        });
        ...
        listStore.sort(function (a, b) {
            ...
            if (index) {
                ...
                $("list-store").insert('<li onmouseover="'+index.getAttribute('onmouseover')+'" onmouseout="'+index.getAttribute('onmouseout')+'" id="' + index.getAttribute('id') + '" class="item" ' + style + '>' + index.innerHTML + '</li>');
            }
        });
    }
