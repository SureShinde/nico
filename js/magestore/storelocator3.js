...
MapManager.prototype = {
    ...
    filterbyTag: function() {
        ...
        if (this.circle.getMap()) {
            if (arrayIds.length == 1 && arrayIds[0] == "") {
                this.listStoreInCircle.each((function(marker) {
                    this.showMarker(marker);
                }).bind(this));
            } else {
                this.listStoreInCircle.each((function(marker) {
    ...
    getOptionFilterByArea: function() {
        ...
        if (inputState && selectState) {
            ...
            if (inputState.style.display == 'none' && selectState.value != '') {
                options['state_id'] = selectState.value;
        ...
        if (address === '') {
            ...
            this.geocoder.geocode({
                ...
                if (status === google.maps.GeocoderStatus.OK) {
                    ...
                    this.listStoreInCircle = [];
                    this.markers.each((function(el) {
                        if (el.radius <= radius1) {
                            this.showMarker(el);
                            this.listStoreInCircle.push(el);
                        }
                    }).bind(this));
                    this.filterbyTag();
                    this.updateCountLabel();
            ...
            navigator.geolocation.getCurrentPosition(function(position) {
                ...
                geocoder.geocode({
                    ...
                    if (status == google.maps.GeocoderStatus.OK) {
                        ...
                        this.codeAddress(results[0]['formatted_address'], radius);
                    }
                }.bind(this));
